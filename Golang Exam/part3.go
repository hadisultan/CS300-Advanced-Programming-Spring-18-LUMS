...
func parallel_decompress(codes []int, dict[][]int, size chan<- int, slice <-chan []int) {
    ...
    if(codesize<=16) {
        slicesize := 0
        for i:= 0; i<codesize; i++ {
            slicesize = slicesize + len(dict[codes[i]])
        }
        size<- slicesize // tell parent size
        td:= <-slice // get slice from parent
        pointer := 0
        for i := 0; i < codesize; i++ {
            temparr := dict[codes[i]]
            varlen := len(temparr)
        ...
        left := make(chan int)
        right := make(chan int)
        slicel := make(chan []int)
        slicer := make(chan []int)
        go parallel_decompress(codes[:mid], dict, left, slicel)
        go parallel_decompress(codes[mid:], dict, right, slicer)
        leftsize := <-left
        rightsize := <-right
        total := leftsize + rightsize
        size<- total
        td := <-slice
        leftslice := td[:leftsize]
        rightslice := td[leftsize:]
        slicel <-leftslice
        slicer <-rightslice
        <-left
        <-right
